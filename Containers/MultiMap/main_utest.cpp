...
TEST(MultiMapInt, CopyConstructor)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(1, 1));
...
TEST(MultiMapInt, RangeConstructor)
{
    const int size = 6;
    std::pair<int, int> arr[size] = {
       std::make_pair(4, 4), 
    ...
    MultiMap<int, int> s(arr, arr + size);
    std::stringstream ss;
    s.print(ss);
    EXPECT_EQ(ss.str(), "{ 1:1 1:1 4:4 4:4 5:5 6:6 }");
}

TEST(MultiMapInt, EraseIterator1)
{
    MultiMap<int, int> s;
...
TEST(MultiMapInt, EraseIterator2)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(4, 4));
    s.insert(std::make_pair(1, 1));
...
TEST(MultiMapInt, EraseIterator3)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(4, 4));
    std::cout << s << std::endl;
...
TEST(MultiMapInt, RangeErase)
{
    ...
    MultiMap<int, int>::iterator it = s.begin();
    MultiMap<int, int>::iterator jt = s.end();
    s.erase(it, jt);
    s.print();
}

TEST(MultiMapInt, Big_RangeErase)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(4, 4));
    s.insert(std::make_pair(1, 1));
...
TEST(MultiMapInt, decrement)
{
    ...
    s.insert(std::make_pair(19, 19));
    std::cout << s << "\n\n" << std::endl;
    MultiMap<int, int>::const_iterator first = s.begin();
    while (first != s.end()) {
        std::cout << first->first << ":" << first->second << ' ';
        ++first;
    } 
    std::cout << std::endl;
    MultiMap<int, int>::const_iterator last = s.find(19); 
    while (last != s.end()) {
        std::cout << last->first << ":" << last->second << ' ';
        --last;
    } 
    std::cout << std::endl;
    MultiMap<int, int>::reverse_iterator b = s.rbegin();
    while (b != s.rend()) { std::cout << (*b).first << (*b) .second << ' '; ++b; }
    b = s.rbegin();
    std::cout << (*(b.operator->())).first << ":" << (*(b.operator->())).second << std::endl;    
}

TEST(MultiMapInt, bounds)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(3, 3));
    s.insert(std::make_pair(2, 2));
...
TEST(MultiMapInt, eraseKey5)
{
    ...
    std::cout << s[33] << std::endl;
    std::cout << s << "\n\n" << std::endl;
    std::pair<MultiMap<int, int>::iterator, bool> pt = s.insert(std::make_pair(9, 8));
    std::cout << std::boolalpha;
    std::cout << *pt.first << ":" << pt.second << std::endl;
    std::cout << s << "\n\n" << std::endl;
}

TEST(MultiMapInt, bound)
{
    MultiMap<int, int> s;
    s.insert(std::make_pair(10, 10));
    s.insert(std::make_pair(5, 5));
...
TEST(MultiMapInt, bound2)
{
    MultiMap<double, double> s;
    s.insert(std::make_pair(10, 10));
    s.insert(std::make_pair(5, 5));
    s.insert(std::make_pair(15, 15));
    s.insert(std::make_pair(2, 2));
...
TEST(MultiMapStringInt, multi)
{
    MultiMap<std::string, int> s;
    s.insert(std::make_pair("a", 10));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("b", 5));
    s.insert(std::make_pair("c", 5));
    s.insert(std::make_pair("d", 5));
    s.insert(std::make_pair("e", 5));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("a", 5));
    s.insert(std::make_pair("a", 6));
    s.insert(std::make_pair("f", 0));
    s.insert(std::make_pair("g", 5));
    s.insert(std::make_pair("h", 15));
    s.insert(std::make_pair("b", 2));
    s.insert(std::make_pair("a", 8));
    s.insert(std::make_pair("a", 12));
    s.insert(std::make_pair("v", 18));
    s.insert(std::make_pair("c", 0));
    s.insert(std::make_pair("r", 3));
    s.insert(std::make_pair("aa", 6));
    s.insert(std::make_pair("ab", 9));
    s.insert(std::make_pair("ac", 11));
    s.insert(std::make_pair("aabc", 13));
    s.insert(std::make_pair("bba", 16));
    s.insert(std::make_pair("b", 19));
    std::cout << s << "\n\n" << std::endl;
    typedef MultiMap<std::string, int>::iterator It;
    It first = s.lower_bound("f");
    It last = s.upper_bound("f");
    std::cout << *first << std::endl;
    std::cout << *last << std::endl;
    for (It it = first; it != last; ++it) {
        std::cout << *it << ' ';
    ...
    std::cout << "count(b): " << s.count("b") << std::endl;
    std::cout << s << "\n\n" << std::endl;
    
    
    std::cout << "afsghfjahsgdljfhagsdjlfhagsdljfhgaldsjhfgaljdshgfljsdhgflajhgfljahdgsljfhl" << std::endl; 
    std::cout << "total number of a: " << s.erase("a") << std::endl;
    std::cout << s << "\n\n" << std::endl;
    std::cout << "total number of b: " << s.erase("b") << std::endl;
    std::cout << s << "\n\n" << std::endl;
}

TEST(MultiMapStringIntTest, Test)
{
    MultiMap<std::string, int> multimap;
    multimap.insert(std::make_pair("apple", 1));
    multimap.insert(std::make_pair("banana", 2));
    multimap.insert(std::make_pair("apple", 3));
    multimap.insert(std::make_pair("orange", 4));
    
    EXPECT_FALSE(multimap.empty());
    EXPECT_EQ(multimap.size(), 4);
    
    multimap.insert(std::make_pair("pear", 5));
    EXPECT_EQ(multimap.size(), 5);
    EXPECT_EQ(multimap.count("pear"), 1);
    
    multimap.insert(std::make_pair("apple", 4)); 
    EXPECT_EQ(multimap.count("apple"), 3);  
    EXPECT_EQ(multimap.count("apple"), 3);  
    EXPECT_EQ(multimap.count("banana"), 1); 
    EXPECT_EQ(multimap.count("pear"), 1);   
    
    EXPECT_EQ(multimap.size(), 6);
    EXPECT_EQ(multimap.count("apple"), 3);
    EXPECT_EQ(multimap.erase("apple"), 3);
    EXPECT_EQ(multimap.count("apple"), 0);
    EXPECT_EQ(multimap.size(), 3);
    
    multimap.insert(std::make_pair("apple", 2)); 
    multimap.insert(std::make_pair("apple", 2)); 
    std::pair<MultiMap<std::string, int>::iterator, MultiMap<std::string, int>::iterator > range = multimap.equal_range("apple");
    /// EXPECT_EQ(std::distance(range.first, range.second), 2); 

    std::cout << "range.first: " << range.first->first << std::endl;
    std::cout << "range.second: " << range.second->first << std::endl;
    int sum = 0;
    for (MultiMap<std::string, int>::iterator it = range.first; it != range.second; ++it) {
        sum += it->second;
    }
    EXPECT_EQ(sum, 4);
   
    std::cout << multimap << std::endl; 
    MultiMap<std::string, int>::iterator it = multimap.begin();
    ...
    for (MultiMap<std::string, int>::iterator entry = multimap.begin(); entry != multimap.end(); ++entry) {
        if (entry->first == "apple") {
            found_apple = true;
        }
        if (entry->first == "banana") {
            found_banana = true;
        }
    }
