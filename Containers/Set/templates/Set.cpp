...
{
    if (empty()) { root_ = new Node(x); return std::pair<iterator, bool>(begin(), true); }
    goUp(it, x);
    const bool isAdded = goDownAndInsert(it, x);
    balance(it);
    return std::pair<iterator, bool>(it, isAdded);
}
...
{
    if (!root) { return std::make_pair<iterator, bool>(root, false); }
    if (*root == key) { return std::make_pair<iterator, bool>(root, true); }
    if (key < *root && root.left())  { return boundHelper(root.left(), key);  } 
    if (key > *root && root.right()) { return boundHelper(root.right(), key); }
    return std::make_pair<iterator, bool>(root, false);
}
...
Set<Data>::operator==(const Set& rhv) const
{
    ...
    while (first1 != end() && first2 != rhv.end()) {
...
template <typename Data>
void 
Set<Data>::levelOrderIter(std::ostream& out) const
{
    Queue<Node*> que;
    que.push(root_);
    while (!que.empty()) {
