#ifndef __MULTI_SET_T_HPP__
#define __MULTI_SET_T_HPP__
...
class MultiSet 
{
    template <typename T> friend std::ostream& operator<<(std::ostream& out, const MultiSet<T>& rhv);
private:
    struct Node {
...
public:
    ...
    ~MultiSet();
    const MultiSet& operator=(const MultiSet& rhv);
